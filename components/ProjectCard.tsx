...
export function ProjectCard({
  ...
  const links = projectLinks.flatMap((entry) => {
    const href = project.links?.[entry.value];
      ...
      <article className="group relative overflow-hidden rounded-[2.5rem] border border-[#d7c7bc]/35 bg-bg-surface text-text-on-light shadow-[0_24px_80px_rgba(9,2,4,0.18)] transition duration-300 ease-out hover:-translate-y-1 hover:shadow-[0_32px_95px_rgba(9,2,4,0.24)]">
        ...
        <div className="relative grid gap-8 p-5 sm:p-6 xl:grid-cols-[minmax(17rem,0.88fr)_minmax(0,1fr)] xl:items-center xl:gap-10 xl:p-7">
          ...
          <div className="flex h-full flex-col">
            ...
            <p className="mt-6 text-xs uppercase tracking-[0.28em] text-text-on-light/55">
              Case Study
            </p>
            ...
            <ul className="mt-8 flex flex-wrap gap-2" aria-label={`${project.title} tags`}>
            ...
            <div className="mt-10 flex flex-wrap gap-3">
              {links.map((link) => (
                <Link
                  ...
                  className="inline-flex items-center rounded-full border border-[#2b0a0f]/12 bg-[#2b0a0f]/4 px-4 py-2 text-sm font-medium text-text-on-light transition duration-200 ease-out hover:bg-[#2b0a0f]/8"
                >
                  {link.label}
                </Link>
              ))}
  ...
  return (
    <article className="group relative overflow-hidden rounded-[2.2rem] border border-white/10 bg-white/[0.04] p-4 shadow-panel backdrop-blur-sm transition duration-300 ease-out hover:-translate-y-1 hover:bg-white/[0.06]">
      ...
      <div className="relative">
        ...
        <ul className="mt-7 flex flex-wrap gap-2" aria-label={`${project.title} tags`}>
          {project.tags.map((tag) => (
            <li
              key={tag}
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          {links.map((link) => (
            <Link
              ...
              target={isExternalHref(link.href) ? "_blank" : undefined}
              rel={isExternalHref(link.href) ? "noreferrer" : undefined}
              className="inline-flex items-center rounded-full border border-white/10 bg-white/[0.05] px-4 py-2 text-sm font-medium text-text-primary transition duration-200 ease-out hover:bg-white/[0.09]"
            >
              {link.label}
            </Link>
          ))}
